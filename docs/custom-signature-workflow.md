...
7. After the write flag is deliberately enabled, `POST /api/custom-signature-sets/{id}/export` requires `ENABLE WRITE` and imports the selected rule IDs into a new user-defined NetScaler signature object over the allow-listed SSH CLI path, then saves the ADC configuration.
