...
### Positive security phase 5: passive evidence fusion and draft generation

- Timestamp: 2026-09-22 UTC
- Implementation: added `POST /api/discovery/jobs/{job_id}/positive-model-candidate` and a GUI action to fuse a completed scan into a saved, editable positive-model draft. The generator uses observed same-host GET/HEAD routes and successful runtime GET/HEAD API metadata, preserves their distinct evidence-source labels, adds technology-detection and optional read-only ADC-topology references, and carries generic protection applicability scores into the review summary.
- Safety: static JavaScript route/auth candidates are counted as unverified gaps and never promoted to endpoints; cross-host routes and non-GET/HEAD methods are excluded; query values are stripped; draft endpoints remain `discovered`/`observe`; no ADC write or target request is made by draft generation. Operator authentication, same-origin/CSRF, size limits, and candidate audit use the existing protected persistence workflow.
- Validation: 20 focused positive-model, generic-applicability, and validator tests pass; Python source compilation, inline frontend JavaScript syntax, and `git diff --check` pass. Live deployment at revision `22827b2` verified all eight services running, Postgres healthy, control API `/healthz` `ok`, HTTPS GUI `200` with the new action present, and unauthenticated API access rejected with `401`. The new API route is registered. The operator-auth test module could not run with the bundled Python because `httpx` is absent there.
- Deployment safety: no candidate draft was persisted during smoke checks and no ADC write endpoint was called. The existing `NETSCALER_WRITE_ENABLED` values in both control-api and netscaler-adapter were found to be `true`; they were not changed as part of the GUI deployment. Treat this as an outstanding security/configuration item and do not exercise any write workflow until an operator explicitly confirms its intended state.
- Repository state: implementation commit `22827b2` is pushed to `origin/main` and deployed at `/home/grega/waf-intelligence-repo`; Postgres volume and Compose secrets were preserved. Compose also rebuilt/recreated the `discovery-worker` and `netscaler-adapter` dependencies; all eight services returned to running state. The GUI/data plane is currently on `22827b2`.
- Recovery after connectivity loss or reboot: verify local and server revisions are `22827b2` or later; use `C:\Users\G.Zoubek\Projects\waf-intelligence` locally and `grega@192.168.11.90:/home/grega/waf-intelligence-repo` remotely. For remote Compose commands, obtain `WAF_DB_PASSWORD` in a shell variable from the already-running Postgres container environment without printing it; never remove the `postgres-data` volume or replace secret files. Check Compose status, `/healthz`, and `https://192.168.11.90/`; confirm anonymous API requests remain `401` and the draft-generation control is visible after operator login. Do not generate a persistent smoke candidate or call ADC write endpoints without explicit approval; the existing write flags are `true` and were not modified.
