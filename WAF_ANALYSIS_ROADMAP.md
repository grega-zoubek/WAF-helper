...
- Validation: 20 focused positive-model, generic-applicability, and validator tests pass; Python source compilation, inline frontend JavaScript syntax, and `git diff --check` pass. The operator-auth test module could not run with the bundled Python because `httpx` is absent there. Live API/browser validation and deployment are pending.
- Repository state: local uncommitted worktree based on `96af288`; no commit or push has been made. The server was not changed.
- Recovery after connectivity loss or reboot: return to `C:\Users\G.Zoubek\Projects\waf-intelligence`, verify `git rev-parse --short HEAD` is `96af288`, and inspect `git status --short` before continuing; preserve the uncommitted edits (do not reset/clean). Rerun `python -m unittest tests.test_positive_learning tests.test_positive_model tests.test_positive_validator tests.test_generic_protection`. Before deployment, commit/push only after review, then on `grega@192.168.11.90` use `/home/grega/waf-intelligence-repo`, pull the approved revision, preserve all Compose secrets and Postgres volumes, rebuild/recreate only `control-api` and its frontend-serving service, check `/healthz` and HTTPS GUI, then create a draft from an approved completed scan and verify its sources, observe-only mode, and excluded static candidates. Do not enable ADC writes or fetch static candidates during recovery.
